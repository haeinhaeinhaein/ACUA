...
const downloadSaveModal = document.getElementById("downloadSaveModal");
const downloadSaveButton = document.getElementById("downloadSaveButton");
const downloadSaveCancel = document.getElementById("downloadSaveCancel");
const downloadSaveFilename = document.getElementById("downloadSaveFilename");
...
/**
 * macOS Safari 등: 사용자 클릭이 끝난 뒤(await 이후) 자동 다운로드가 무시되는 경우가 있음.
 * iOS는 Web Share 경로를 쓰므로 제외한다.
 */
function prefersExplicitDownloadFallback() {
  if (typeof navigator === "undefined") return false;
  if (isIosLikeTouchDevice()) return false;
  const ua = navigator.userAgent || "";
  const isChromium = /\b(Chrome|Chromium|Edg|OPR)\//i.test(ua);
  if (isChromium) return false;
  const isFirefox = /\bFirefox\//i.test(ua);
  const isSafari = /\bSafari\//i.test(ua) && !/\b(Chrome|Chromium|Edg|OPR)\//i.test(ua);
...
/**
 * Safari 등에서 비동기 작업 후에도 유효한 "저장" 제스처로 저장한다.
 */
function offerExplicitSaveModal(blob, filename) {
  return new Promise((resolve) => {
    ...
    const finish = (saved) => {
      ...
      settled = true;
      cleanup();
      document.body.classList.remove("download-save-active");
      if (downloadSaveModal) downloadSaveModal.classList.remove("is-visible");
      window.setTimeout(() => {
        if (downloadSaveModal) downloadSaveModal.classList.add("hidden");
    ...
    const onBackdrop = (event) => {
      ...
      if (target instanceof HTMLElement && target.dataset.downloadSave === "backdrop") finish(false);
    };
    const onEscape = (event) => {
      if (event.key !== "Escape") return;
      if (!downloadSaveModal || !downloadSaveModal.classList.contains("is-visible")) return;
      event.preventDefault();
      finish(false);
    };
    function cleanup() {
      if (!downloadSaveButton || !downloadSaveCancel || !downloadSaveModal) return;
      downloadSaveButton.removeEventListener("click", onSaveClick);
      downloadSaveCancel.removeEventListener("click", onCancelClick);
      downloadSaveModal.removeEventListener("click", onBackdrop);
      document.removeEventListener("keydown", onEscape);
    }
    if (!downloadSaveModal || !downloadSaveButton || !downloadSaveCancel || !downloadSaveFilename) {
      resolve(false);
      return;
    }
    downloadSaveFilename.textContent = filename;
    document.body.classList.add("download-save-active");
    downloadSaveModal.classList.remove("hidden");
    downloadSaveModal.offsetHeight;
    requestAnimationFrame(() => {
      downloadSaveModal.classList.add("is-visible");
      try {
        downloadSaveButton.focus({ preventScroll: true });
      } catch (_) {
        downloadSaveButton.focus();
      }
    });
    downloadSaveButton.addEventListener("click", onSaveClick);
    downloadSaveCancel.addEventListener("click", onCancelClick);
    downloadSaveModal.addEventListener("click", onBackdrop);
...
async function exportReportAsImage() {
  ...
  const exportBasename = buildExportBasenameFromSaveTime();
  const pngName = `${exportBasename}.png`;
  let saveHandle = null;
  if (canUseSaveFilePicker()) {
    try {
      saveHandle = await window.showSaveFilePicker({
        suggestedName: pngName,
  ...
  try {
    ...
    if (prefersExplicitDownloadFallback()) {
      await offerExplicitSaveModal(blob, pngName);
      return;
    }
    await offerFileToUser(blob, pngName, "image/png", {
...
async function exportReportAsDocument() {
  ...
  const exportBasename = buildExportBasenameFromSaveTime();
  const htmlName = `${exportBasename}.html`;
  let saveHandle = null;
  if (canUseSaveFilePicker()) {
    try {
      saveHandle = await window.showSaveFilePicker({
  ...
  if (prefersExplicitDownloadFallback()) {
    await offerExplicitSaveModal(blob, htmlName);
    return;
  }
  await offerFileToUser(blob, htmlName, "text/html");
}
